  ...
  return (
    <div className="border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden">
      <button
        type="button"
        onClick={onToggle}
        className="w-full flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-[#33363d] hover:bg-[#FFFCE6] dark:hover:bg-white/5 transition-colors text-left"
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {day.summary && (
              <p className="text-xs text-gray-500 dark:text-gray-300 mt-0.5 truncate">{day.summary}</p>
            )}
          </div>
        </div>
        {isOpen
      ...
      {isOpen && (
        <div className="px-4 py-3.5 bg-white dark:bg-[#3a3d44] space-y-3 border-t border-gray-100 dark:border-white/10 dark:border-white/10">
              ...
              return (
                <div key={label}>
                  ...
                  <ul className="mt-1 space-y-1">
                    {parts.map((line, i) => (
                      <li key={i} className="text-sm text-gray-700 dark:text-gray-100 leading-relaxed">
                        {line}
                      </li>
                    ))}
                  </ul>
          ...
          {day.hotel?.name && (
            <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-white/10">
              <Building2 size={12} className="text-gray-400 dark:text-gray-400 flex-shrink-0" />
...
export default function ItineraryDisplay({ itinerary, sessionId, onSave, onDiscard, saved }) {
  ...
  return (
    <div className="w-full space-y-3">
      ...
      {/* ── Itinerary Summary ── */}
      {(itinerary.itinerary_summary || itinerary.overview) && (
        <div className="bg-gray-50 dark:bg-[#33363d] rounded-xl px-5 py-4 border border-gray-100 dark:border-white/10">
          <p className="text-[11px] font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-2">Itinerary Summary</p>
          <p className="text-sm text-gray-700 dark:text-gray-100 leading-relaxed">{itinerary.itinerary_summary || itinerary.overview}</p>
        </div>
      )}
      ...
      {itinerary.days?.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider">
            ...
            <div className="flex gap-1.5">
              <button
                ...
                className="text-[10px] text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:text-gray-200 px-2 py-0.5 rounded hover:bg-gray-100 transition-colors"
              >
                Expand all
              </button>
              <button
      ...
      {hasCost && (
        <div>
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-2 hover:text-gray-700 dark:text-gray-100 transition-colors w-full text-left"
          >
            Cost Breakdown
                ...
                return (
                  <div
                    ...
                    className={`flex justify-between px-4 py-2.5 text-sm ${
                      i % 2 === 0 ? "bg-white dark:bg-[#3a3d44]" : "bg-gray-50 dark:bg-[#33363d]"
                    } ${isTotal ? "font-semibold border-t-2 border-[#FFDE39] bg-[#FFFCE6]!" : ""}`}
                  >
                    <span className={`capitalize ${isTotal ? "text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-200"}`}>
                      {k.replace(/_/g, " ")}
                    </span>
                    <span className={`font-medium ${isTotal ? "text-gray-900 dark:text-white" : "text-gray-800 dark:text-white"}`}>
                      {v}
                    </span>
                  </div>
                );
      ...
      {hotels.length > 0 && (
        <div>
          <p className="text-[11px] font-bold text-gray-500 dark:text-gray-300 uppercase tracking-wider mb-2">
            Hotel Options
          </p>
          <div className="border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden divide-y divide-gray-100 dark:divide-white/10">
            {hotels.slice(0, 5).map((h, i) => {
              ...
              return (
                <div key={i} className="flex justify-between items-center px-4 py-2.5 bg-white dark:bg-[#3a3d44] hover:bg-[#FFFCE6] dark:hover:bg-white/5 transition-colors">
                  <div className="min-w-0">
                    <span className="text-sm font-medium text-gray-800 dark:text-white truncate">{h.name}</span>
                    {(h.city || h.area) && (
                  ...
                  {price && (
                    <span className="text-xs text-gray-600 dark:text-gray-200 font-medium ml-3 flex-shrink-0">{price}</span>
                  )}
                </div>
              );
            })}
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {inclusions.map((inc, i) => (
                  <li key={i} className="flex gap-2 text-xs text-gray-600 dark:text-gray-200 leading-relaxed">
      ...
      {/* ── Notes ── */}
      {itinerary.notes && (
        <p className="text-xs text-gray-500 dark:text-gray-300 italic border-l-2 border-gray-200 dark:border-white/15 pl-3">
          {itinerary.notes}
        </p>
      )}
